...
public class PilotStudyExpression extends MentalistCoreExpression implements ExpressionPolicy{
	...
	public String getExpression(History history) 
	{
		...
		if(last.getType().equals(Event.EventClass.SEND_EXPRESSION)){
			if(last.getMessage().equals("sad"))
				...
				return "happy";
			else if(last.getMessage().equals("surprised") || last.getMessage().equals("neutral"))
				return "neutral";
