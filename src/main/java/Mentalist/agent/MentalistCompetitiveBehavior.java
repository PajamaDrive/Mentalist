...
public class MentalistCompetitiveBehavior extends MentalistCoreBehavior implements BehaviorPolicy {
...
//			ServletUtils.log("Actual Value: " + utils.myActualOfferValue(propose) + " vs. Split: " + (fairSplit - getAcceptMargin()), ServletUtils.DebugLevels.DEBUG);
			ServletUtils.log("Considering new offer to propose...", DebugLevels.DEBUG);
			if(!acceptOffer(propose)) // Don't go below what we'd formally accept
			{
				ServletUtils.log("New Offer rejected. Returning to most recent offer.", DebugLevels.DEBUG);
			...
			{
				ServletUtils.log("New offer accepted.", DebugLevels.DEBUG);
				concession.setOffer(propose);
			}
		}

		opponentValue = utils.getAdversaryValue(propose);
		ServletUtils.log("Ensuring new offer is fair to opponent...", DebugLevels.DEBUG);
	...
	private Offer getTimingOffer()
	{
		...
		ServletUtils.log("Agent rolling dice: " + ((double)((int)(chance*100)))/100, DebugLevels.DEBUG);
		if (chance > 0.50)
			return null;
		else
			return getNextOffer();
	...
	protected boolean acceptOffer(Offer o)
	{
		...
		ServletUtils.log("acceptOffer method - Agent Value: " + myValue + ", Perceived Opponent Value: " + opponentValue + ", Opponent BATNA: " + utils.adversaryBATNA, DebugLevels.DEBUG);
